...
function prometheus_host_windows_physcial_disk_write_bytes_monitoring(data) {
	...
	if (data.snmpData != undefined || data.snmpData != null) {
		...
		for (i = 0; i < counter_result.length; i++) {
			...
			var diskOutBytes = ((counter_result[i]["wmi_logical_disk_write_bytes_total"] / sectorSize)*1024);
			...
			pointBuilder.addTagData("metricName", "disk-write-bytes");
			...
			pointBuilder.addFieldData("diskOutBytes", diskOutBytes);
			metricName = 'disk-write-bytes'
			data.points.add(pointBuilder);
			data.scriptOutputs.put("diskOutBytes", diskOutBytes);
