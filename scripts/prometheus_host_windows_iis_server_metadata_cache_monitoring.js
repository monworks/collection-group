...
function prometheus_host_windows_iis_server_metadata_cache_monitoring(data) {
	var count = 0;
	...
	if (data.snmpData != undefined || data.snmpData != null) {
		...
		var counter_result = diffCounter(merge_result, ["time", "assetId", "assetLabel", "customerId"]);
		...
		for (i = 0; i < counter_result.length; i++) {
			...
			var file_cache_items_total = counter_result[i]["wmi_iis_server_metadata_cache_items_cached_total"];
            var metricNameIT = "MetaCache"
			...
			var file_cache_hits_total = counter_result[i]["wmi_iis_server_metadata_cache_hits_total"];
            var metricNameHits = "MetaCache";
