...
function prometheus_host_mysql_innodb_metrics_monitoring(data) {
	...
	if (data.snmpData != undefined || data.snmpData != null) {
		...
		for (i = 0; i < counter_result.length; i++) {
		    ...
		    if(counter_result[i]["mysql_info_schema_innodb_metrics_lock_lock_row_lock_time_total"] >=0){
			var lockTime = counter_result[i]["mysql_info_schema_innodb_metrics_lock_lock_row_lock_time_total"];
           
			var metricNamemd = "lock-row";

			var componentId1 = "time"
			...
			pointBuilder.addData("metricValue", lockTime);
			pointBuilder.addData("metricName", metricNamemd);
			pointBuilder.addData("componentId", componentId1);
			data.points.add(pointBuilder);

			data.scriptOutputs.put("metricValue", lockTime);
			data.scriptOutputs.put("componentId", componentId1);
		    }
		
	        if (counter_result[i]["mysql_info_schema_innodb_metrics_lock_lock_row_lock_waits_total"] >=0) {
			var lockWait = counter_result[i]["mysql_info_schema_innodb_metrics_lock_lock_row_lock_waits_total"];
           
			var metricNamemd = "lock-row";

			var componentId2= "wait"
			...
			// pointBuilder.addData("name", name);
			pointBuilder.addData("componentIdentifier", name);
			pointBuilder.addData("metricValue", lockWait);
			pointBuilder.addData("metricName", metricNamemd);
			pointBuilder.addData("componentId", componentId2);
			data.points.add(pointBuilder);

			data.scriptOutputs.put("metricValue", lockWait);
